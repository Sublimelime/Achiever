data:extend{
	{
		type = "achievement",
		name = "friendly-fire",
		order = "g",
		icon = "__Achiever__/graphics/friendly-fire.png"
	},
	{
		type = "achievement",
		name = "tango-down",
		order = "h",
		icon = "__Achiever__/graphics/tango-down.png"
	},
	{
		type = "achievement",
		name = "looted",
		order = "i",
		icon = "__Achiever__/graphics/looted.png"
	},
	{
		type = "achievement",
		name = "copy-and-pasted",
		order = "j",
		icon = "__Achiever__/graphics/copy-and-pasted.png"
	},
	{
		type = "achievement",
		name = "depleted",
		order = "l",
		icon = "__Achiever__/graphics/depleted.png"
	},
	{
		type = "achievement",
		name = "hello-world",
		order = "m",
		icon = "__Achiever__/graphics/hello-world.png"
	},
	{
		type = "achievement",
		name = "deforestation",
		order = "n",
		icon = "__Achiever__/graphics/deforestation.png"
	},
	{
		type = "achievement",
		name = "fully-automatic",
		order = "o",
		icon = "__Achiever__/graphics/fully-automatic.png"
	},
	{
		type = "achievement",
		name = "let-there-be-light",
		order = "o",
		icon = "__Achiever__/graphics/let-there-be-light.png"
	},
	{
		type = "achievement",
		name = "praise-the-sun",
		order = "o",
		icon = "__Achiever__/graphics/praise-the-sun.png"
	},
	{
		type = "achievement",
		name = "dedication",
		order = "s",
		icon = "__Achiever__/graphics/dedication.png"
	},
	{
		type = "achievement",
		name = "maggots",
		order = "u",
		icon = "__Achiever__/graphics/maggots.png"
	},
	{
		type = "achievement",
		name = "we-didnt-start-the-fire",
		order = "z",
		icon = "__Achiever__/graphics/we-didnt-start-the-fire.png"
	},
	{
		type = "achievement",
		name = "you-forgot-something",
		order = "v",
		icon = "__Achiever__/graphics/you-forgot-something.png"
	},
	{
		type = "achievement",
		name = "ghosted",
		order = "k",
		icon = "__Achiever__/graphics/ghosted.png"
	},
	{
		type = "achievement",
		name = "blueprinted",
		order = "t",
		icon = "__Achiever__/graphics/blueprinted.png"
	},
	{
		type = "achievement",
		name = "convey-your-ideas",
		order = "l",
		icon = "__Achiever__/graphics/convey-your-ideas.png"
	},
}
